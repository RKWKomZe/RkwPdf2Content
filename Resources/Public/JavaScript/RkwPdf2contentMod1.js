define([
  ...
  var _errorMessage = function (message) {
    ...
    var configuration = {
      title: TYPO3.lang['be.js.label.error'],
      content: message,
  ...
  var _initPDF = function () {
    ...
    // create form data
    console.log(_ajaxProcessPdfUrl);
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result) {
          ...
          _errorMessage(TYPO3.lang['be.js.msg.error_process_begin'] + ': ' + result.message);
        }
      },
      error: function (jqXHR, textStatus, errorThrown) {
        $('.overlay', '.editor').addClass('hidden');
        _errorMessage(TYPO3.lang['be.js.msg.error_ajax_request'] + ' ' + textStatus + ' ' + errorThrown);
      }
    });
  ...
  $('.submit_upload').on('click', function (evt) {
    ...
    var configuration = {
      title: 'Init PDF',
      content: TYPO3.lang['be.js.msg.confirm_init'],
  ...
  $('.file_pdf_upload', _upload_form).on('change', function (evt) {
    ...
    if (_validateFile(file) === true) {
      ...
      _errorMessage(TYPO3.lang['be.js.msg.error_pdf_file']);
    }
  });
  ...
  $('.submit_render', _renderForm).on('click', function (evt) {
    ...
    if (_validateRenderSubmit() === true) {
      ...
      var configuration = {
        title: 'Process PDF',
        content: TYPO3.lang['be.js.msg.confirm_render'],
        severity: Severity.info,
        url: _ajaxProcessPdfUrl,
        buttons: [
          {
  ...
  var _validateRenderSubmit = function () {
    ...
    if ($('input.target_page_id', _renderForm).val() == '') {
      _errorMessage(TYPO3.lang['be.js.msg.error_target_page']);
      return false;
    }

    // target page title set?
    if ($('input.first_page_title', _renderForm).val() == '') {
      _errorMessage(TYPO3.lang['be.js.msg.error_page_title']);
      return false;
    }

    // pdf initialized?
    if (_pdfInitialized !== true) {
      _errorMessage(TYPO3.lang['be.js.msg.error_pdf_not_init']);
      return false;
    }

    // tree set?
    if ($('input.tree_payload', _renderForm).val() == '[]') {
      _errorMessage(TYPO3.lang['be.js.msg.error_empty_doc']);
      return false;
    }
